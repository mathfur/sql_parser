...
spec = do
  describe "" $ do
    it "" $ (Parser.to_sql "SELECT id,name FROM users")
       `shouldBe`
       (Right $ SQL [SelectStmt (SelectCore [ResultColumn "id", ResultColumn "name"] (JoinSource (TableNameSingleSource (TableName Nothing "users") Nothing) [])) Nothing])
    it "" $ (Parser.to_sql "SELECT id, name FROM users")
       `shouldBe`
       (Right $ SQL [SelectStmt (SelectCore [ResultColumn "id", ResultColumn "name"] (JoinSource (TableNameSingleSource (TableName Nothing "users") Nothing) [])) Nothing])
    ...
    it "" $ (Parser.to_sql "SELECT * FROM users LIMIT 1")
       `shouldBe`
       (Right $ SQL [SelectStmt (SelectCore [ResultColumn "*"] (JoinSource (TableNameSingleSource (TableName Nothing "users") Nothing) [])) (Just (LimitTerm (Expr "1") Nothing))])
