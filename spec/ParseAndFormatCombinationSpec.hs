...
spec = do
    describe "Main parse functionality" $ do
        ...
        it "" $ assert_of_to_sql_and_format "SELECT DISTINCT 1 FROM users"
        ...
        it "" $ assert_of_to_sql_and_format "SELECT DISTINCT count(*) FROM users"
        it "" $ assert_of_to_sql_and_format "SELECT ALL count(*) FROM users"
